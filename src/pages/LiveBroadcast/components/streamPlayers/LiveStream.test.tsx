...
vi.mock('./StreamDisplay', () => ({
	default: vi.fn(() => (
		<div data-testid="stream-display" />
	)),
}));
...
describe.skip('LiveStream Component', () => {
	...
	test('renders with light theme URL when theme mode is light', () => {
		...
		expect(display).toHaveAttribute('data-src', 'https://embeds.sermoncloud.com/new-york-united/live?theme=light');
	});
	...
	test('renders with dark theme URL when theme mode is dark', () => {
		...
		const display = screen.getByTestId('stream-display');
		expect(display).toHaveAttribute('data-src', 'https://embeds.sermoncloud.com/new-york-united/live?theme=dark');
	});
});
