...
const watchRouteList = useMemo(() => {
	let results: string[] = [];
	routes.filter((i) => (i?.path && i?.path.indexOf('watch') > -1)).forEach((i) => {
		
		if (i.children) i.children.forEach((ch) => ch?.path && results.push(ch?.path));	
...
useEffect(() => {
	if(pathname.indexOf('watch') > -1) {
		...
		if(filteredRoutes.length === 0 ) navigate(watchRouteList[0], { replace: true });
	}
