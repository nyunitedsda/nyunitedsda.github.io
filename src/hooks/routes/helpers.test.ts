...
describe("formatRoutes", () => {
  it("formats a simple route", () => {
    ...
    expect(output).toEqual([
      { path: "/home", element: "<Home />", caseSensitive: false, errorElement: undefined }
    ]);
  });

  it("formats an index route", () => {
    const input = [
      { index: true, element: "<Index />", name: "Index" }
    ];
    const output = formatRoutes(input);
    expect(output).toEqual([
    ...
    const input = [
      {
        ...
        element: "<Parent />",
        children: [
          { path: "child", element: "<Child />", name: "Child" }
        ]
      }
    ];
    const output = formatRoutes(input);
    expect(output).toEqual([
      {
        ...
        children: [
          { path: "child", element: "<Child />", caseSensitive: false, errorElement: undefined }
        ]
      }
    ]);
  });
