...
describe("CountdownTimer", () => {
  ...
  it("renders correct initial values for a future target date", () => {
    ...
    expect(screen.getByText("03").nextSibling).toBe(screen.getByText('Minutes'));
    expect(screen.getByText("04").nextSibling).toBe(screen.getByText('Seconds'));
  });
  ...
  it("updates the timer after 1 second", async () => {
    ...
    expect(screen.getByText("10").nextSibling).toBe(screen.getByText('Seconds'));
