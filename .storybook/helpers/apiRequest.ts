...
const createEntityHandler = <T extends { id?: number }>({url, data, entity}: EntityHandlerParams<T>) => {
  if (!url || !data || !entity) {
    throw new Error("URL, data, and entity must be provided");
  }

let localData: T[] = [...data];
  ...
  return [
    ...
    http.get(`${url}/:id`, (req) => {
      ...
      if (!item) {
        return new HttpResponse({ status: 404, body: { error: `${entity} not found` } });
      }
    ...
    http.post(`${url}`, async (req) => {
      ...
      if (typeof body !== "object" || body === null) {
        return new HttpResponse({ status: 400, body: { error: "Invalid request body" } });
      }
      // Find max id
    ...
    http.put(`${url}/:id`, async (req) => {
      ...
      if (typeof localData[idx] === "object" && localData[idx] !== null && typeof body === "object" && body !== null) {
        localData[idx] = { ...localData[idx], ...body };
      } else {
        return new HttpResponse({ status: 400, body: { error: "Invalid data for update" } });
      }
